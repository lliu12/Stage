...
int FiducialUpdate(ModelFiducial *fid, robot_t *robot)
{
  robot->stop = false;
  int nearby_robots = 0;
  radians_t total_nearby_robots_angle = 0;
  ...
  FOR_EACH (it, fid->GetFiducials()) {
    ...
    if (other->range < robot->stopdist) {
      robot->stop = true;
      nearby_robots++;
      total_nearby_robots_angle += other->geom.a;
      // printf("neighboring robot heading error: %f", other->geom.a);
    }
  }
  ...
  if (nearby_robots > 0) {
    robot->avg_nbr_heading_error = total_nearby_robots_angle / nearby_robots;
    // printf("avg neighboring robot heading error: %f \n\n", robot->avg_nbr_heading_error);
  }
...
{
  // printf("%s at ", robot->pos->Token());
  // robot->pos->GetPose().Print(" ");
  ...
  if (robot->current_phase_count == 0) {
    ...
    if (!robot->running && robot->current_phase_count == 0) {
      ...
      if (robot->closest) {
        ...
        Stg::radians_t avg_angle = normalize(.5 * (robot->avg_nbr_heading_error + 2 * robot->pos->GetPose().a));
        
        // float frustration = (robot->memory_length == 0 ? 1 : float(robot->blocked_count) / float(robot->memory_length));
        frustration = (frustration > 0.5 ? 1 : 0);

        robot->goal_angle = normalize((1 - (1-frustration)) * (goal_angle + distribution(generator)) + (1-frustration) * avg_angle);
