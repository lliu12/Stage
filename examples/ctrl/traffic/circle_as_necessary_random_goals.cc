...
// goal positions randomized on circle edge
...
static const double circle_radius = 3; // the circle that all agents' start and goal positions lie on
...
extern "C" int Init(Model *mod, CtrlArgs *)
{
  ...
  Pose circle_center = robot->pos->GetPose() + Pose(circle_radius,0,0,0); // assumes robot is initialized on circle edge, facing center
  double rand_angle = drand48() * 2 * M_PI;
  robot->goal_pos = circle_center + Pose(circle_radius * cos(rand_angle),circle_radius * sin(rand_angle),0,0);
